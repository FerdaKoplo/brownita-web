                ...
                <tbody class="bg-white divide-y divide-gray-200 text-xs sm:text-sm text-gray-700">
                    @forelse ($transaksis as $index => $transaksi)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-green-600 font-medium">Rp {{ number_format($transaksi->total_harga, 0, ',', '.') }}</td>
                            <td class="px-3 sm:px-4 py-2 sm:py-3">
                                @if(request('edit') == $transaksi->id)
                                    <form action="{{ route('dashboard.admin.customer-transaction.update', $transaksi->id) }}"
                                        ...
                                        <select name="status" onchange="this.form.submit()"
                                            class="text-xs sm:text-sm rounded px-2 py-1 border bg-white shadow-sm">
                            ...
                            <td class="px-3 sm:px-4 py-2 sm:py-3 space-x-2">
