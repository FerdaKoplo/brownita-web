    ...
    <div class="flex flex-col lg:flex-row px-4 md:px-16 lg:px-32 gap-12 justify-center min-h-screen items-center">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-6 sm:gap-16 w-full bg-white rounded-2xl shadow-md p-5">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-6">
                        <div class="flex flex-col gap-2 items-center sm:items-start">
                            ...
                            <div
                                class="flex bg-amber-700 rounded-xl gap-8 sm:gap-12 py-1  items-center justify-between text-white ">
                                <form method="POST" action="{{ route('customer.keranjang.update', $cartitem->id) }}">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="action" value="decrease">
                                    <button type="submit"
                                        class="w-8 h-8 rounded-full flex items-center justify-center hover:bg-black transition">-</button>
                                </form>
        ...
        <form method="POST" action="{{ route('customer.transaksi.store') }}" class="flex flex-col gap-8 w-full lg:w-2/5">
            ...
            <div class="flex flex-col gap-4">
                ...
                <textarea name="alamat" cols="30" rows="5" placeholder="Ketik alamat yang sesuai..."
